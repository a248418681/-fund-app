...
class SearchBloc extends Bloc<SearchEvent, SearchState> {
  ...
  Future<void> _onQueryChanged(SearchQueryChanged event, Emitter<SearchState> emit) async {
    ...
    try {
      ...
      // 缓存热门基金
      final hotFunds = state.hotFunds.isEmpty ? results.take(20).toList() : state.hotFunds;
  ...
  void _onFundSelected(SearchFundSelected event, Emitter<SearchState> emit) {
    final fund = state.results.firstWhere(
      (f) => f.code == event.code,
      orElse: () => FundInfo(code: event.code, name: event.name, type: '', pinyin: ''),
    );
  ...
  static final List<FundInfo> _defaultHotFunds = [
    FundInfo(code: '005827', name: '易方达蓝筹精选混合', type: '混合型', pinyin: 'yifangda'),
    FundInfo(code: '110022', name: '招商中证白酒指数(LOF)A', type: '指数型', pinyin: 'zhaoshang'),
    FundInfo(code: '012348', name: '天弘恒生科技指数(QDII)C', type: 'QDII', pinyin: 'tianhong'),
    FundInfo(code: '001548', name: '招商国证生物医药指数(LOF)', type: '指数型', pinyin: 'zhaoshang'),
    FundInfo(code: '320007', name: '诺安成长混合', type: '混合型', pinyin: 'nuoan'),
    FundInfo(code: '162703', name: '广发中证传媒ETF联接A', type: '指数型', pinyin: 'guangfa'),
    FundInfo(code: '006328', name: '中泰星元灵活配置混合A', type: '混合型', pinyin: 'zhongtai'),
    FundInfo(code: '000961', name: '天弘沪深300ETF联接A', type: '指数型', pinyin: 'tianhong'),
    FundInfo(code: '159915', name: '易方达创业板ETF', type: '指数型', pinyin: 'yifangda'),
    FundInfo(code: '513500', name: '博时标普500ETF联接A', type: 'QDII', pinyin: 'bosera'),
  ];
